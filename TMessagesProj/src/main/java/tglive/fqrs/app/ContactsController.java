...
public class ContactsController extends BaseController {
    ...
    public void checkAppAccount() {
        ...
        Utilities.globalQueue.postRunnable(() -> {
            ...
            try {
                Account[] accounts = am.getAccountsByType("tglive.fqrs.app");
            ...
            if (getUserConfig().isClientActivated()) {
                ...
                if (systemAccount == null) {
                    try {
                        systemAccount = new Account("" + getUserConfig().getClientUserId(), "tglive.fqrs.app");
    ...
    protected void performSyncPhoneBook(final HashMap<String, Contact> contactHashMap, final boolean request, final boolean first, final boolean schedule, final boolean force, final boolean checkCount, final boolean canceled) {
        ...
        Utilities.globalQueue.postRunnable(() -> {
            ...
            /*if (schedule) {
                try {
                    AccountManager am = AccountManager.get(ApplicationLoader.applicationContext);
                    Account[] accounts = am.getAccountsByType("tglive.fqrs.app.account");
                    boolean recreateAccount = false;
                    if (getUserConfig().isClientActivated()) {
                        if (accounts.length != 1) {
                            FileLog.e("detected account deletion!");
                            currentAccount = new Account(getUserConfig().getCurrentUser().phone, "tglive.fqrs.app.account");
...
//                });
    ...
    private void applyContactToPhoneBook(ArrayList<ContentProviderOperation> query, TLRPC.User user) {
        ...
        builder.withValue(ContactsContract.Data.MIMETYPE, "vnd.android.cursor.item/vnd.tglive.fqrs.app.android.profile");
        ...
        builder = ContentProviderOperation.newInsert(ContactsContract.Data.CONTENT_URI);
        builder.withValueBackReference(ContactsContract.Data.RAW_CONTACT_ID, rawContactId);
        builder.withValue(ContactsContract.Data.MIMETYPE, "vnd.android.cursor.item/vnd.tglive.fqrs.app.android.call");
        builder.withValue(ContactsContract.Data.DATA1, user.id);
        ...
        builder = ContentProviderOperation.newInsert(ContactsContract.Data.CONTENT_URI);
        builder.withValueBackReference(ContactsContract.Data.RAW_CONTACT_ID, rawContactId);
        builder.withValue(ContactsContract.Data.MIMETYPE, "vnd.android.cursor.item/vnd.tglive.fqrs.app.android.call.video");
        builder.withValue(ContactsContract.Data.DATA1, user.id);
