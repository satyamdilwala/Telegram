...
import tglive.fqrs.app.SQLite.SQLiteCursor;
import tglive.fqrs.app.SQLite.SQLiteDatabase;
import tglive.fqrs.app.SQLite.SQLiteException;
import tglive.fqrs.app.SQLite.SQLitePreparedStatement;
import tglive.fqrs.app.ringtone.RingtoneDataStore;
import tglive.fqrs.app.ringtone.RingtoneUploader;
import tglive.fqrs.app.tgnet.ConnectionsManager;
import tglive.fqrs.app.tgnet.NativeByteBuffer;
...
import tglive.fqrs.app.ui.Components.ChatThemeBottomSheet;
...
import tglive.fqrs.app.ui.Components.StickersArchiveAlert;
...
import tglive.fqrs.app.ui.Stories.StoriesStorage;
...
public class MediaDataController extends BaseController {
  ...
  public static String SHORTCUT_CATEGORY = "tglive.fqrs.app.SHORTCUT_SHARE";
  ...
  private void processLoadedStickers(int type, ArrayList<TLRPC.TL_messages_stickerSet> res, boolean cache, int date, long hash, Runnable onFinish) {
    ...
    Utilities.stageQueue.postRunnable(() -> {
      if (cache && (res == null || BuildVars.DEBUG_VERSION || Math.abs(System.currentTimeMillis() / 1000 - date) >= 60 * 60) || !cache && res == null && hash == 0) {
        AndroidUtilities.runOnUIThread(() -> {
          if (res != null && hash != 0) {
            loadHash[type] = hash;
          }
      ...
      getMessagesStorage().getStorageQueue().postRunnable(() -> {
        ...
        try {
          ...
          AndroidUtilities.runOnUIThread(() -> {
            ...
            getNotificationCenter().postNotificationName(NotificationCenter.reloadInlineHints);
            getNotificationCenter().postNotificationName(NotificationCenter.reloadWebappsHints);
            if (Math.abs(getUserConfig().lastHintsSyncTime - (int) (System.currentTimeMillis() / 1000)) >= 24 * 60 * 60 || BuildVars.DEBUG_VERSION) {
              loadHints(false);
            }
          });
        } catch (Exception e) {
...
//      }
  ...
  private void loadAvatarConstructor(boolean profile) {
    ...
    if (emojiList == null || (System.currentTimeMillis() - lastCheckTime) > 24 * 60 * 60 * 1000 || BuildVars.DEBUG_VERSION) {
