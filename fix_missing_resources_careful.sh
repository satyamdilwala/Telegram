...
echo "=== Carefully Fixing Missing Raw Resources ==="
...
for resource in "${MISSING_RESOURCES[@]}"; do
    ...
    for file in $files; do
        ...
        # Only comment out complete lines that contain the resource reference
        # This is safer than commenting out partial lines
        sed -i.tmp "s/^\([[:space:]]*\)\(.*R\.raw\.$resource.*\)$/\1\/\/ \2/g" "$file"
        rm "$file.tmp"
    done
done

echo "=== Done! ==="
echo "All missing resource references have been commented out (complete lines only)."
